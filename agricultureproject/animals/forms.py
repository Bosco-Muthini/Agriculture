...
from .models import Cattle,CattleSex,Machinery,Goat, Sheep
...
class SheepForm(forms.ModelForm):
    class Meta:
        model=Sheep
        fields=('owner_contact','breed','weight','maximum_selling_price','location','sheep_age','sheep_sex_based_on','health_record_book','sheep_picture')
        # fields='__all__'
        labels={
            'maximum_selling_price':'Maximum Selling Price',
            'sheep_age':'Sheep Age In Years',
            'weight':'Weight In Kilograms',
            'location':'County Location',
            'sheep_sex_based_on':'Sheep Age & Sex Based On',
        }
    def __init__(self,*args,**kwargs,): 
        super(SheepForm,self).__init__(*args,**kwargs)
        self.fields['sheep_sex_based_on'].empty_label='select'
